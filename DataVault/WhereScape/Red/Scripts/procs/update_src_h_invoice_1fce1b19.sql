...
-- Procedure Name   :    update_src_h_invoice_1fce1b19
...
-- Generated on     :    Wednesday, March 08, 2023 at 11:24:06
-- Author           :    Paul Cafarella
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_src_h_invoice_1fce1b19].update_src_h_invoice_1fce1b19
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_invoice]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_invoice_paceport.hk_h_invoice AS hk_h_invoice
           , stage_invoice_paceport.invoice_no AS invoice_no
           , stage_invoice_paceport.invoice_no_bkcc AS invoice_no_bkcc
           , stage_invoice_paceport.tenant_id AS tenant_id
           , stage_invoice_paceport.dss_record_source AS dss_record_source
           , stage_invoice_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_invoice_paceport] stage_invoice_paceport
    WHERE NOT EXISTS (
        SELECT 1
        FROM   [TABLEOWNER].[h_invoice] h_invoice
        WHERE  stage_invoice_paceport.invoice_no = h_invoice.invoice_no
        AND    stage_invoice_paceport.invoice_no_bkcc = h_invoice.invoice_no_bkcc
        AND    stage_invoice_paceport.tenant_id = h_invoice.tenant_id
        )
    ;
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_src_h_invoice_1fce1b19 FAILED with error '
