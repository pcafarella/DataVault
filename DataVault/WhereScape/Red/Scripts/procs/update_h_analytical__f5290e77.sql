...
-- Procedure Name   :    update_h_analytical__f5290e77
...
-- Generated on     :    Friday, August 11, 2023 at 17:12:59
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_h_analytical__f5290e77].update_h_analytical__f5290e77
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_analytical_instrument]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_analytical_instrument.hk_h_analytical_instrument AS hk_h_analytical_instrument
           , stage_analytical_instrument.instrument_no AS instrument_no
           , stage_analytical_instrument.instrument_no_bkcc AS instrument_no_bkcc
           , stage_analytical_instrument.tenant_id AS tenant_id
           , stage_analytical_instrument.dss_record_source AS dss_record_source
           , stage_analytical_instrument.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_analytical_instrument] stage_analytical_instrument
    WHERE NOT EXISTS (
        SELECT 1
        FROM   [TABLEOWNER].[h_analytical_instrument] h_analytical_instrument
        WHERE  stage_analytical_instrument.instrument_no = h_analytical_instrument.instrument_no
        AND    stage_analytical_instrument.instrument_no_bkcc = h_analytical_instrument.instrument_no_bkcc
        AND    stage_analytical_instrument.tenant_id = h_analytical_instrument.tenant_id
        )
    ;
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_h_analytical__f5290e77 FAILED with error '
