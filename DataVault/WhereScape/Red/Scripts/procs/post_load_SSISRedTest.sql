...
--
 CREATE   PROCEDURE [dbo].[post_load_SSISRedTest]
   @p_sequence integer
...
SELECT @environment_name = 'testenv'--CASE WHEN @environment_name IS NULL THEN 1 ELSE @environment_name END
...
BEGIN
	...
	SET @p_return_msg = 'Unhandled Exception in WS_RUN_SSIS_Catalog_Package at step '      + CONVERT(varchar,@v_step) +  SUBSTRING(@v_db_msg,1,150)
...
SET @v_step = 200
SET @p_status = 1
SET @p_return_msg = 'WS_RUN_SSIS_Catalog_Package completed. '    + @v_return_msg
