...
-- Procedure Name   :    update_h_department_9397c789
...
-- Generated on     :    Friday, July 07, 2023 at 12:45:55
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_h_department_9397c789].update_h_department_9397c789
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_department]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_department.hk_h_department AS hk_h_department
           , stage_department.department_no AS department_no
           , stage_department.department_no_bkcc AS department_no_bkcc
           , stage_department.tenant_id AS tenant_id
           , stage_department.dss_record_source AS dss_record_source
           , stage_department.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_department] stage_department
    WHERE NOT EXISTS (
        SELECT 1
        FROM   [TABLEOWNER].[h_department] h_department
        WHERE  stage_department.department_no = h_department.department_no
        AND    stage_department.department_no_bkcc = h_department.department_no_bkcc
        AND    stage_department.tenant_id = h_department.tenant_id
        )
    ;
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_h_department_9397c789 FAILED with error '
