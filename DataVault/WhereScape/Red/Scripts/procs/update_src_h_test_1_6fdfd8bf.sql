...
-- Procedure Name   :    update_src_h_test_1_6fdfd8bf
...
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_src_h_test_1_6fdfd8bf].update_src_h_test_1_6fdfd8bf
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_test]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_test_paceport.hk_h_test AS hk_h_test
           , stage_test_paceport.test_no AS test_no
           , stage_test_paceport.test_no_bkcc AS test_no_bkcc
           , stage_test_paceport.tenant_id AS tenant_id
           , stage_test_paceport.dss_record_source AS dss_record_source
           , stage_test_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_test_paceport] stage_test_paceport
    WHERE NOT EXISTS (
        SELECT 1
        FROM   [TABLEOWNER].[h_test] h_test
        WHERE  stage_test_paceport.test_no = h_test.test_no
        AND    stage_test_paceport.test_no_bkcc = h_test.test_no_bkcc
        AND    stage_test_paceport.tenant_id = h_test.tenant_id
        )
    ;
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_src_h_test_1_6fdfd8bf FAILED with error '
