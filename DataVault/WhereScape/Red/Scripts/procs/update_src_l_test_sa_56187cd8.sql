...
-- Procedure Name   :    update_src_l_test_sa_56187cd8
...
-- Generated on     :    Friday, March 17, 2023 at 13:15:54
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_src_l_test_sa_56187cd8].update_src_l_test_sa_56187cd8
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[l_test_sample]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_test_sample_paceport.hk_l_test_sample AS hk_l_test_sample
           , stage_test_sample_paceport.hk_h_test AS hk_h_test
           , stage_test_sample_paceport.hk_h_sample AS hk_h_sample
           , stage_test_sample_paceport.dss_record_source AS dss_record_source
           , stage_test_sample_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_test_sample_paceport] stage_test_sample_paceport
    WHERE NOT EXISTS (
        SELECT 1
        FROM   [TABLEOWNER].[l_test_sample] l_test_sample
        WHERE  stage_test_sample_paceport.hk_h_sample = l_test_sample.hk_h_sample
        AND    stage_test_sample_paceport.hk_h_test = l_test_sample.hk_h_test
        )
    ;
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_src_l_test_sa_56187cd8 FAILED with error '
