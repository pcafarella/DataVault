...
-- Procedure Name   :    update_src_h_analyti_9c480c77
...
-- Generated on     :    Friday, August 11, 2023 at 17:46:29
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_src_h_analyti_9c480c77].update_src_h_analyti_9c480c77
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_analytical_instrument]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_analytical_instrument_paceport.hk_h_analytical_instrument AS hk_h_analytical_instrument
           , stage_analytical_instrument_paceport.instrument_no AS instrument_no
           , CAST(NULL AS NVARCHAR(50)) AS instrument_no_bkcc
           , CAST(NULL AS NVARCHAR(255)) AS tenant_id
           , stage_analytical_instrument_paceport.dss_record_source AS dss_record_source
           , stage_analytical_instrument_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_analytical_instrument_paceport] stage_analytical_instrument_paceport
    WHERE NOT EXISTS (
        SELECT 1
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_src_h_analyti_9c480c77 FAILED with error '
