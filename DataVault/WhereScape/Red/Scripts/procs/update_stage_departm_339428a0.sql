...
-- Procedure Name   :    update_stage_departm_339428a0
-- Template         :    wsl_sqlserver_proc_dv_stage_sha256
-- Template Version :    8.5.1.0
-- Description      :    Update the Stage Table table stage_department_paceport
-- Generated by     :    WhereScape RED Version 8.6.6.1 (build 220216-231917)
-- Generated for    :    Pace Analytical Services
-- Generated on     :    Wednesday, September 13, 2023 at 16:48:16
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_stage_departm_339428a0].update_stage_departm_339428a0
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    SET @v_sql = N'TRUNCATE TABLE [TABLEOWNER].[stage_department_paceport]';
    ...
    BEGIN TRANSACTION
      ...
      INSERT INTO [TABLEOWNER].[stage_department_paceport]
      ( hk_h_department
      , department_no
      , department_no_bkcc
      , tenant_id
      ...
      SELECT  CONVERT(NVARCHAR(64),HashBytes('SHA2_256',
               COALESCE(CAST(load_dmart_locations_paceport.location_seq AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST(ISNULL(system_id, '-1') AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST(ISNULL(system_id, '-1') AS VARCHAR(MAX)),'null')
               ),2) AS hk_h_department
           , load_dmart_locations_paceport.location_seq AS department_no
           , ISNULL(system_id, '-1') AS department_no_bkcc
           , ISNULL(system_id, '-1') AS tenant_id
           , load_dmart_locations_paceport.dss_record_source AS dss_record_source
           , load_dmart_locations_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
      FROM [TABLEOWNER].[load_dmart_locations_paceport] load_dmart_locations_paceport
      ;
    ...
    SET @p_status = 1
    SET @p_return_msg = 'stage_department_paceport updated. '
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_stage_departm_339428a0 FAILED with error '
