...
-- Procedure Name   :    update_src_h_analysi_d44011b3
...
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_src_h_analysi_d44011b3].update_src_h_analysi_d44011b3
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_analysis_process]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_analysis_process_paceport.hk_h_analysis_process AS hk_h_analysis_process
           , stage_analysis_process_paceport.analysis_process_code AS analysis_process_code
           , CAST(NULL AS NVARCHAR(50)) AS analysis_process_code_bkcc
           , CAST(NULL AS NVARCHAR(255)) AS tenant_id
           , stage_analysis_process_paceport.dss_record_source AS dss_record_source
           , stage_analysis_process_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_analysis_process_paceport] stage_analysis_process_paceport
    WHERE NOT EXISTS (
        SELECT 1
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_src_h_analysi_d44011b3 FAILED with error '
