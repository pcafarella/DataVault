...
-- Procedure Name   :    update_stage_custome_19fdc114
-- Template         :    wsl_sqlserver_proc_dv_stage_sha256
-- Template Version :    8.5.1.0
-- Description      :    Update the Stage Table table stage_customer_workorder_national
-- Generated by     :    WhereScape RED Version 8.6.6.1 (build 220216-231917)
-- Generated for    :    Pace Analytical Services
-- Generated on     :    Wednesday, March 08, 2023 at 10:44:45
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_stage_custome_19fdc114].update_stage_custome_19fdc114
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    SET @v_sql = N'TRUNCATE TABLE [TABLEOWNER].[stage_customer_workorder_national]';
    ...
    BEGIN TRANSACTION
      ...
      INSERT INTO [TABLEOWNER].[stage_customer_workorder_national]
      ...
      SELECT  CONVERT(NVARCHAR(64),HashBytes('SHA2_256',
               COALESCE(CAST(ISNULL(loginnum,'-1') AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST('lims80' AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST('lims80' AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST(ISNULL(acctnum,'-1') AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST('lims80' AS VARCHAR(MAX)),'null') +'||'+
               COALESCE(CAST('lims80' AS VARCHAR(MAX)),'null')
               ),2) AS hk_l_customer_workorder
           ...
           , ISNULL(acctnum,'-1') AS account_no
           , 'lims80' AS account_no_bkcc
           , 'lims80' AS account_no_tenant_id
           , ISNULL(loginnum,'-1') AS work_order_no
      ...
      FROM [TABLEOWNER].[load_vw_sdg_details_national] load_vw_sdg_details_national
      ;
    ...
    SET @p_status = 1
    SET @p_return_msg = 'stage_customer_workorder_national updated. '
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_stage_custome_19fdc114 FAILED with error '
