...
-- Procedure Name   :    update_src_h_sample__ce34f072
...
-- Generated on     :    Wednesday, March 08, 2023 at 10:46:38
-- Author           :
--==============================================================================
-- Notes / History
--
CREATE PROCEDURE [SCHEMA.update_src_h_sample__ce34f072].update_src_h_sample__ce34f072
  @p_sequence         INTEGER
...
AS
  ...
  BEGIN TRY
    ...
    INSERT INTO [TABLEOWNER].[h_sample]
      ...
      , dss_load_date
      , dss_create_time)
    SELECT DISTINCT stage_sample_paceport.hk_h_sample AS hk_h_sample
           , stage_sample_paceport.sample_no AS sample_no
           , stage_sample_paceport.sample_no_bkcc AS sample_no_bkcc
           , stage_sample_paceport.tenant_id AS tenant_id
           , stage_sample_paceport.dss_record_source AS dss_record_source
           , stage_sample_paceport.dss_load_date AS dss_load_date
           , @v_current_datetime AS dss_create_time
    FROM [TABLEOWNER].[stage_sample_paceport] stage_sample_paceport
    WHERE NOT EXISTS (
        SELECT 1
        FROM   [TABLEOWNER].[h_sample] h_sample
        WHERE  stage_sample_paceport.sample_no = h_sample.sample_no
        AND    stage_sample_paceport.sample_no_bkcc = h_sample.sample_no_bkcc
        AND    stage_sample_paceport.tenant_id = h_sample.tenant_id
        )
    ;
  ...
  BEGIN CATCH
    ...
    SET @p_status = -2
    SET @p_return_msg = SUBSTRING('update_src_h_sample__ce34f072 FAILED with error '
