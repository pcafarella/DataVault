/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_netimpact_erfree]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_netimpact_erfree]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_netimpact_erfree]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
GO
CREATE     PROCEDURE [info].[usp_fact_invoiceline_netimpact_erfree] AS 

IF object_id('tempdb.dbo.#temp') iS NOT NULL DROP TABLE #temp
IF object_id('info.fact_invoiceline_netimpact_erfree') iS NOT NULL DROP TABLE info.fact_invoiceline_netimpact_erfree

--exec info.usp_fact_invoiceline_netimpact_erfree
...
SELECT	COUNT(*) [Count],
        ...
        Product, 
 		SUM(CASE WHEN [Revenue Adjusted] > 0 THEN Volume ELSE Volume * -1 END)  Volume,
		SUM([Revenue Adjusted]) [Revenue Adjusted]	
FROM  info.fact_invoiceline_erfree
LEFT JOIN info.dim_lab ON dim_lab.[Lab Text] = CAST(fact_invoiceline_erfree.lab AS VARCHAR)
WHERE [is Owner or Standard] = 1 
AND [accounting type] = 'Invoice'
...
SELECT   CAST(ISNULL([ASP CY],0) AS Decimal(38,2)) [ASP CY]
		...
		,CASE WHEN ISNULL([Lag Volume],0.00)  = 0 THEN [Revenue Adjusted] 
			  ...
			  END [Volume Impact]
INTO info.fact_invoiceline_netimpact_erfree
FROM (		 
SELECT * 
