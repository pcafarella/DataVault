/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_eastlongmeadow]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_eastlongmeadow]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_eastlongmeadow]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE               PROCEDURE [info].[usp_fact_invoiceline_eastlongmeadow] AS 
 
-- exec info.usp_fact_invoiceline_eastlongmeadow

IF OBJECT_ID('info.fact_invoiceline_eastlongmeadow_ancillary') IS NOT NULL DROP TABLE info.fact_invoiceline_eastlongmeadow_ancillary
IF OBJECT_ID('info.fact_invoiceline_eastlongmeadow_credit') IS NOT NULL DROP TABLE info.fact_invoiceline_eastlongmeadow_credit
IF OBJECT_ID('info.fact_invoiceline_eastlongmeadow_invoice') IS NOT NULL DROP TABLE info.fact_invoiceline_eastlongmeadow_invoice 

DECLARE @dss_snapshot_date DATETIME, @invoice_no varchar(50)
SELECT @dss_snapshot_date = (SELECT MAX(dss_snapshot_date) from bus.bridge_test_invoiceline_eastlongmeadow)
...
SELECT CASE WHEN (stw.price * stw.surcharge/ 100) + stw.price < 0 THEN 'Credit' ELSE 'Invoice' END [Accounting Type],
		 ...
		 COALESCE(NULLIF(sprw.UserInfo5,''),'-1') Industry,
 ...
 INTO info.fact_invoiceline_eastlongmeadow_invoice
 --select top 1000 h_pace_account.*
FROM  bus.bridge_test_invoiceline_eastlongmeadow b
...
LEFT OUTER JOIN bus.s_test_hroc_eastlongmeadow_current stw ON stw.hk_h_test = b.hk_h_test -- 1043445
LEFT OUTER JOIN bus.s_work_order_mroc_eastlongmeadow_current sww ON sww.hk_h_work_order = b.hk_h_work_order
LEFT OUTER JOIN bus.s_sample_mroc_eastlongmeadow_current ssw ON ssw.hk_h_sample = b.hk_h_sample
LEFT OUTER JOIN bus.s_invoice_mroc_eastlongmeadow_current siw ON siw.hk_h_invoice = b.hk_h_invoice
LEFT OUTER JOIN bus.s_project_mroc_eastlongmeadow_current sprw ON sprw.hk_h_project = b.hk_h_project
...
SELECT  CASE WHEN quantity < 0 THEN 'Credit' ELSE 'Invoice' END   [Accounting Type],
		 ...
		 'lims57-Ancillary' [Source Name],
		'-1' TAT,
	    'placeholder' [TAT Code],
		 ABS(quantity) Volume, 
		 saiw.ItemType +CASE WHEN NULLIF(itemcode,'') is not null  then '|'+saiw.itemcode else '' END [Word Order Desc],
		 CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 	
INTO info.fact_invoiceline_eastlongmeadow_ancillary
FROM bus.bridge_test_invoiceline_eastlongmeadow b
INNER JOIN raw.h_invoice h_invoice On h_invoice.hk_h_invoice = b.hk_h_invoice
LEFT OUTER JOIN raw.h_pace_account h_pace_account ON h_pace_account.hk_h_pace_account= b.hk_h_pace_account
LEFT OUTER JOIN raw.h_customer h_customer On h_customer.hk_h_customer = b.hk_h_customer
LEFT OUTER JOIN bus.s_invoice_mroc_eastlongmeadow_current siw ON siw.hk_h_invoice =b.hk_h_invoice
LEFT OUTER JOIN bus.s_ancillary_invoiceitem_tran_eastlongmeadow_current saiw ON saiw.hk_l_ancillary_invoiceitem = b.hk_l_ancillary_invoiceitem
LEFT OUTER JOIN bus.s_project_mroc_eastlongmeadow_current sprw ON sprw.hk_h_project = b.hk_h_project
LEFT OUTER JOIN bus.s_sample_mroc_eastlongmeadow_current ssw ON ssw.hk_h_sample = b.hk_h_sample
LEFT OUTER JOIN mas.customer_interregional cir ON LTRIM(RTRIM(cir.ir_customer_no)) = LTRIM(RTRIM(h_pace_account.pace_account_no)) 
WHERE b.tenant_id = 'lims57'
...
SELECT   'Credit' [Accounting Type],
		 ...
		 'lims57-Credit' [Source Name],
	 	 '-1' TAT,
	     'placeholder' [TAT Code],
		  1 Volume, 
		  ISNULL(h_test.test_no,'-1')+'|'+h_work_order.work_order_no [Word Order Desc],
		  CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 
INTO info.fact_invoiceline_eastlongmeadow_credit 
FROM bus.bridge_test_invoiceline_eastlongmeadow b
INNER JOIN raw.h_invoice h_invoice On  h_invoice.hk_h_invoice = b.hk_h_invoice
...
LEFT OUTER JOIN bus.s_invoice_mroc_eastlongmeadow_current siw ON siw.hk_h_invoice = b.hk_h_invoice
LEFT OUTER JOIN bus.s_work_order_mroc_eastlongmeadow_current sww ON sww.hk_h_work_order = b.hk_h_work_order
LEFT OUTER JOIN bus.s_test_hroc_eastlongmeadow_current stm ON stm.hk_h_test =b.hk_h_test
LEFT OUTER JOIN bus.s_test_invoice_credit_tran_eastlongmeadow_current saiw ON saiw.hk_l_test_invoice_credit = b.hk_l_test_invoice_credit
LEFT OUTER JOIN bus.s_project_mroc_eastlongmeadow_current sprw ON sprw.hk_h_project = b.hk_h_project
LEFT OUTER JOIN bus.s_sample_mroc_eastlongmeadow_current ssw ON ssw.hk_h_sample = b.hk_h_sample
LEFT OUTER JOIN mas.customer_interregional cir ON LTRIM(RTRIM(cir.ir_customer_no)) = LTRIM(RTRIM(h_pace_account.pace_account_no)) 
WHERE b.tenant_id = 'lims57'
