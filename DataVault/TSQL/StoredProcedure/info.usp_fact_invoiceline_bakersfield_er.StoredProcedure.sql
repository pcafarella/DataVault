/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_bakersfield_er]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_bakersfield_er]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_bakersfield_er]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
GO
CREATE           PROCEDURE [info].[usp_fact_invoiceline_bakersfield_er] AS 
 
-- exec info.usp_fact_invoiceline_bakersfield_er

IF OBJECT_ID('info.fact_invoiceline_bakersfield_ancillary_er') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_ancillary_er
IF OBJECT_ID('info.fact_invoiceline_bakersfield_credit_er') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_credit_er
IF OBJECT_ID('info.fact_invoiceline_bakersfield_invoice_er') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_invoice_er
...
SELECT CASE WHEN (stw.price * coalesce(stw.surcharge,0)/ 100) + stw.price < 0 THEN 'Credit' ELSE 'Invoice' END [Accounting Type],
 ...
 INTO info.fact_invoiceline_bakersfield_invoice_er
 --select top 1000 *
...
AND COALESCE(b.pace_account_no, '00-000000') IN ('27-101983')
...
SELECT  CASE WHEN saiw.price < 0 OR saiw.quantity < 0 OR saiw.itemtotal < 0 THEN 'Credit' ELSE 'Invoice' END   [Accounting Type],
	    ...
	    'placeholder' [TAT Code],
		 ...
		 CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 	
INTO info.fact_invoiceline_bakersfield_ancillary_er
FROM bus.bridge_test_invoiceline_bakersfield b
...
SELECT   'Credit' [Accounting Type],
	     ...
	     'placeholder' [TAT Code],
		  ...
		  CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 
INTO info.fact_invoiceline_bakersfield_credit_er
FROM bus.bridge_test_invoiceline_bakersfield b
INNER JOIN raw.h_invoice h_invoice On  h_invoice.hk_h_invoice = b.hk_h_invoice
