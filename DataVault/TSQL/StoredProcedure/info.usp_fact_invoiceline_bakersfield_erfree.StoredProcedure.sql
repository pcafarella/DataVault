/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_bakersfield_erfree]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_bakersfield_erfree]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_bakersfield_erfree]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
GO
CREATE              PROCEDURE [info].[usp_fact_invoiceline_bakersfield_erfree] AS 
 
-- exec info.usp_fact_invoiceline_bakersfield_erfree

IF OBJECT_ID('info.fact_invoiceline_bakersfield_ancillary_erfree') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_ancillary_erfree
IF OBJECT_ID('info.fact_invoiceline_bakersfield_credit_erfree') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_credit_erfree
IF OBJECT_ID('info.fact_invoiceline_bakersfield_invoice_erfree') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_invoice_erfree

DECLARE @dss_snapshot_date DATETIME, @invoice_no varchar(50)
SELECT @dss_snapshot_date = (SELECT MAX(dss_snapshot_date) from bus.bridge_test_invoiceline_bakersfield)
...
SELECT CASE WHEN (stw.price * coalesce(stw.surcharge,0)/ 100) + stw.price < 0 THEN 'Credit' ELSE 'Invoice' END [Accounting Type],
 ...
 INTO info.fact_invoiceline_bakersfield_invoice_erfree
 --select top 1000 *
FROM  bus.bridge_test_invoiceline_bakersfield b
...
LEFT OUTER JOIN bus.s_test_hroc_bakersfield_current stw ON stw.hk_h_test = b.hk_h_test -- 1043445
LEFT OUTER JOIN bus.s_work_order_mroc_bakersfield_current sww ON sww.hk_h_work_order = b.hk_h_work_order
LEFT OUTER JOIN bus.s_sample_mroc_bakersfield_current ssw ON ssw.hk_h_sample = b.hk_h_sample
LEFT OUTER JOIN bus.s_invoice_mroc_bakersfield_current siw ON siw.hk_h_invoice = b.hk_h_invoice
LEFT OUTER JOIN bus.s_project_mroc_bakersfield_current sprw ON sprw.hk_h_project = b.hk_h_project
...
AND COALESCE(b.pace_account_no, '00-000000') NOT IN ('44-631225','CA1234','MC1002','27-101983')
...
SELECT  CASE WHEN saiw.price < 0 OR saiw.quantity < 0 OR saiw.itemtotal < 0 THEN 'Credit' ELSE 'Invoice' END   [Accounting Type],
	    ...
	    'placeholder' [TAT Code],
		 ...
		 CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 	
INTO info.fact_invoiceline_bakersfield_ancillary_erfree
FROM bus.bridge_test_invoiceline_bakersfield b
INNER JOIN raw.h_invoice h_invoice On h_invoice.hk_h_invoice = b.hk_h_invoice
LEFT OUTER JOIN raw.h_pace_account h_pace_account ON h_pace_account.hk_h_pace_account= b.hk_h_pace_account
LEFT OUTER JOIN raw.h_customer h_customer On h_customer.hk_h_customer = b.hk_h_customer
LEFT OUTER JOIN bus.s_invoice_mroc_bakersfield_current siw ON siw.hk_h_invoice =b.hk_h_invoice
LEFT OUTER JOIN bus.s_ancillary_invoiceitem_tran_bakersfield_current saiw ON saiw.hk_l_ancillary_invoiceitem = b.hk_l_ancillary_invoiceitem
LEFT OUTER JOIN bus.s_project_mroc_bakersfield_current sprw ON sprw.hk_h_project = b.hk_h_project
LEFT OUTER JOIN bus.s_sample_mroc_bakersfield_current ssw ON ssw.hk_h_sample = b.hk_h_sample
LEFT OUTER JOIN mas.customer_interregional cir ON LTRIM(RTRIM(cir.ir_customer_no)) = LTRIM(RTRIM(h_pace_account.pace_account_no)) 
WHERE b.tenant_id = 'lims27'
...
SELECT   'Credit' [Accounting Type],
	     ...
	     'placeholder' [TAT Code],
		  ...
		  CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 
INTO info.fact_invoiceline_bakersfield_credit_erfree
FROM bus.bridge_test_invoiceline_bakersfield b
INNER JOIN raw.h_invoice h_invoice On  h_invoice.hk_h_invoice = b.hk_h_invoice
...
LEFT OUTER JOIN bus.s_invoice_mroc_bakersfield_current siw ON siw.hk_h_invoice = b.hk_h_invoice
LEFT OUTER JOIN bus.s_work_order_mroc_bakersfield_current sww ON sww.hk_h_work_order = b.hk_h_work_order
LEFT OUTER JOIN bus.s_test_hroc_bakersfield_current stm ON stm.hk_h_test =b.hk_h_test
LEFT OUTER JOIN bus.s_test_invoice_credit_tran_bakersfield_current saiw ON saiw.hk_l_test_invoice_credit = b.hk_l_test_invoice_credit
LEFT OUTER JOIN bus.s_project_mroc_bakersfield_current sprw ON sprw.hk_h_project = b.hk_h_project
LEFT OUTER JOIN bus.s_sample_mroc_bakersfield_current ssw ON ssw.hk_h_sample = b.hk_h_sample
LEFT OUTER JOIN mas.customer_interregional cir ON LTRIM(RTRIM(cir.ir_customer_no)) = LTRIM(RTRIM(h_pace_account.pace_account_no)) 
WHERE b.tenant_id = 'lims27'
