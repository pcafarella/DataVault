/****** Object:  StoredProcedure [bus].[usp_bridge_test_invoiceline_madisonville]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [bus].[usp_bridge_test_invoiceline_madisonville]
GO
/****** Object:  StoredProcedure [bus].[usp_bridge_test_invoiceline_madisonville]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
GO
 ...
 CREATE       PROCEDURE [bus].[usp_bridge_test_invoiceline_madisonville] AS

 -- exec [Bus].[usp_bridge_test_invoiceline_madisonville]

 IF Object_id('bus.bridge_test_invoiceline_madisonville') iS NOT NULL DROP TABLE bus.bridge_test_invoiceline_madisonville
...
SELECT       l_customer_paceaccount.hk_l_customer_paceaccount AS hk_l_customer_paceaccount 
		 ...
		 INTO bus.bridge_test_invoiceline_madisonville
FROM raw.h_invoice
...
LEFT JOIN raw.[l_customer_paceaccount] l_customer_paceaccount ON l_customer_paceaccount.hk_h_customer = h_customer.hk_h_customer
LEFT JOIN raw.[h_pace_account] h_pace_account ON l_customer_paceaccount.hk_h_pace_account = h_pace_account.hk_h_pace_account
...
WHERE h_invoice.tenant_id IN ('lims44')
 
UNION ALL

SELECT       NULL AS hk_l_customer_paceaccount 
...
LEFT JOIN raw.[l_project_workorder] l_project_workorder ON l_project_workorder.hk_h_work_order = h_work_order.hk_h_work_order
LEFT JOIN raw.[h_project] h_project ON l_project_workorder.hk_h_project = h_project.hk_h_project
WHERE h_invoice.tenant_id ='lims44'
