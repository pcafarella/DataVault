...
LEFT OUTER JOIN raw.[l_test_sample] (NOLOCK) l_test_sample ON l_test_sample.hk_h_test = h_test.hk_h_test
LEFT OUTER JOIN raw.[h_sample] (NOLOCK) h_sample ON l_test_sample.hk_h_sample = h_sample.hk_h_sample
LEFT OUTER JOIN raw.[l_test_product] (NOLOCK) l_test_product ON l_test_product.hk_h_test = h_test.hk_h_test
LEFT OUTER JOIN raw.[h_product] (NOLOCK) h_product ON l_test_product.hk_h_product = h_product.hk_h_product
LEFT OUTER JOIN raw.[l_workorder_test] (NOLOCK) l_workorder_test ON l_workorder_test.hk_h_test = h_test.hk_h_test
LEFT OUTER JOIN raw.[h_work_order] (NOLOCK) h_work_order ON l_workorder_test.hk_h_work_order = h_work_order.hk_h_work_order
LEFT OUTER JOIN raw.[l_customer_workorder] (NOLOCK) l_customer_workorder ON l_customer_workorder.hk_h_work_order = h_work_order.hk_h_work_order
LEFT OUTER JOIN raw.[h_customer] (NOLOCK) h_customer  ON h_customer.hk_h_customer = l_customer_workorder.hk_h_customer
LEFT OUTER JOIN raw.[l_customer_paceaccount] (NOLOCK) l_customer_paceaccount ON l_customer_paceaccount.hk_h_customer = h_customer.hk_h_customer
...
LEFT OUTER JOIN raw.[l_project_workorder] (NOLOCK) l_project_workorder ON l_project_workorder.hk_h_work_order = h_work_order.hk_h_work_order
LEFT OUTER JOIN raw.[h_project] (NOLOCK) h_project ON l_project_workorder.hk_h_project = h_project.hk_h_project
