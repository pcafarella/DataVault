/****** Object:  StoredProcedure [bus].[usp_bridge_test_invoiceline_peoria]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [bus].[usp_bridge_test_invoiceline_peoria]
GO
/****** Object:  StoredProcedure [bus].[usp_bridge_test_invoiceline_peoria]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE            PROCEDURE [bus].[usp_bridge_test_invoiceline_peoria] AS

 -- exec [Bus].[usp_bridge_test_invoiceline_peoria]

IF Object_id('bus.bridge_test_invoiceline_peoria') iS NOT NULL DROP TABLE bus.bridge_test_invoiceline_peoria
...
SELECT       l_paceaccount_customer_invoice.hk_l_paceaccount_customer_invoice  
		 ...
		 INTO bus.bridge_test_invoiceline_peoria
FROM raw.h_invoice
...
WHERE h_invoice.tenant_id IN ('lims72')
...
LEFT JOIN raw.[l_project_workorder] l_project_workorder ON l_project_workorder.hk_h_work_order = h_work_order.hk_h_work_order
LEFT JOIN raw.[h_project] h_project ON l_project_workorder.hk_h_project = h_project.hk_h_project
WHERE h_invoice.tenant_id ='lims72'
