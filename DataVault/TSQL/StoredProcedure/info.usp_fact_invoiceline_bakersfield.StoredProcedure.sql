/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_bakersfield]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_bakersfield]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_bakersfield]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
GO
CREATE            PROCEDURE [info].[usp_fact_invoiceline_bakersfield] AS 
 
-- exec info.usp_fact_invoiceline_bakersfield

IF OBJECT_ID('info.fact_invoiceline_bakersfield_ancillary') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_ancillary
IF OBJECT_ID('info.fact_invoiceline_bakersfield_credit') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_credit
IF OBJECT_ID('info.fact_invoiceline_bakersfield_invoice') IS NOT NULL DROP TABLE info.fact_invoiceline_bakersfield_invoice 
...
SELECT CASE WHEN (stw.price * coalesce(stw.surcharge,0)/ 100) + stw.price < 0 THEN 'Credit' ELSE 'Invoice' END [Accounting Type],
 ...
 INTO info.fact_invoiceline_bakersfield_invoice
 --select top 1000 *
...
SELECT  CASE WHEN saiw.price < 0 OR saiw.quantity < 0 OR saiw.itemtotal < 0 THEN 'Credit' ELSE 'Invoice' END   [Accounting Type],
	    ...
	    'placeholder' [TAT Code],
		 ...
		 CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 	
INTO info.fact_invoiceline_bakersfield_ancillary
FROM bus.bridge_test_invoiceline_bakersfield b
...
SELECT   'Credit' [Accounting Type],
	     ...
	     'placeholder' [TAT Code],
		  ...
		  CAST(ROUND(siw.invoicetotal,2) AS DECIMAL(12,2)) total_amt 
INTO info.fact_invoiceline_bakersfield_credit 
FROM bus.bridge_test_invoiceline_bakersfield b
INNER JOIN raw.h_invoice h_invoice On  h_invoice.hk_h_invoice = b.hk_h_invoice
