/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_netimpact_er]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_netimpact_er]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_netimpact_er]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE     PROCEDURE [info].[usp_fact_invoiceline_netimpact_er] AS 

IF object_id('tempdb.dbo.#temp') iS NOT NULL DROP TABLE #temp
IF object_id('info.fact_invoiceline_netimpact_er') iS NOT NULL DROP TABLE info.fact_invoiceline_netimpact_er

--exec info.usp_fact_invoiceline_netimpact_er
...
FROM  info.fact_invoiceline_er 
LEFT JOIN info.dim_lab ON dim_lab.[Lab Text] = CAST(fact_invoiceline_er.lab AS VARCHAR)
...
SELECT   CAST(ISNULL([ASP CY],0) AS Decimal(38,2)) [ASP CY]
		...
		,CASE WHEN ISNULL([Lag Volume],0.00)  = 0 THEN [Revenue Adjusted] 
			  ...
			  END [Volume Impact]
INTO info.fact_invoiceline_netimpact_er
FROM (		 
SELECT * 
