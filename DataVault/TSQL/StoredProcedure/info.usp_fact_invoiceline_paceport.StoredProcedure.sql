/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_paceport]    Script Date: 1/30/2024 2:45:18 PM ******/
DROP PROCEDURE [info].[usp_fact_invoiceline_paceport]
GO
/****** Object:  StoredProcedure [info].[usp_fact_invoiceline_paceport]    Script Date: 1/30/2024 2:45:18 PM ******/
SET ANSI_NULLS ON
GO
...
GO
CREATE   PROCEDURE [info].[usp_fact_invoiceline_paceport] AS 
 
-- exec info.usp_fact_invoiceline_paceport

IF Object_id('info.fact_invoiceline_paceport') IS NOT NULL drop table info.fact_invoiceline_paceport
...
SELECT  CASE WHEN sil.charge >= 0 THEN 'Invoice' ELSE 'Credit' END [Accounting Type],
	   ...
	   'placeholder' [TAT Code],
		...
		0 total_amt
INTO info.fact_invoiceline_paceport
